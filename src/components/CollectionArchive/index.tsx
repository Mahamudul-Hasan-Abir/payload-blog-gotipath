...
export const CollectionArchive: React.FC<Props> = ({
  ...
  return (
    <div className={cn('w-full lg:py-12 md:py-5 py-2 max-w-5xl mx-auto')}>
      <div className={cn('')}>
        ...
        <Tabs defaultValue="all" onValueChange={setSelectedCategory} className="w-full">
          <div className="border rounded-2xl  overflow-hidden mb-8">
            <TabsList className="w-full justify-start p-0 h-auto bg-background overflow-x-auto">
              <TabsTrigger
                value="all"
                className="px-2 py-2 mx-2 my-2 rounded-xl data-[state=active]:bg-black data-[state=active]:text-white whitespace-nowrap"
              >
                All
              </TabsTrigger>
              {categories?.map(
                (
                  ...
                  <TabsTrigger
                    key={category.id}
                    value={category.id}
                    className="mx-2 my-2 py-2 px-2 rounded-xl data-[state=active]:bg-black data-[state=active]:text-white whitespace-nowrap"
                  >
                    {category.title}
                  </TabsTrigger>
                ),
